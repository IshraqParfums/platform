 ...
 * already carries.
 *
 *   colour       FAMILY_PALETTE hues, mixed by how much each family is
...
import { FAMILY_PALETTE } from "./family-colors.js";
...
/** The slice of the retail catalogue this needs. */
export interface CataloguePerfume {
  id: string;
  name: string;
  collection?: string;
...
function blendColour(weights: Partial<Record<Dimension, number>>): string {
  let total = 0;
  let r = 0;
  let g = 0;
  let b = 0;
  for (const dim of DIMENSIONS) {
    ...
    if (w <= 0) continue;
    const [cr, cg, cb] = hexToRgb(FAMILY_PALETTE[dim].accent);
    r += cr * w;
    g += cg * w;
    b += cb * w;
    total += w;
...
function familyWeightsAt(
  ...
  hours: number,
): { families: Partial<Record<Dimension, number>>; total: number } {
  const families: Partial<Record<Dimension, number>> = {};
  let total = 0;
  for (const row of rows) {
    ...
    if (c <= 0) continue;
    total += c;
    for (const [dim, strength] of Object.entries(m.families) as [Dimension, number][]) {
...
function cosine(
  a: Partial<Record<Dimension, number>>,
  b: Partial<Record<Dimension, number>>,
): number {
  let dot = 0;
  let na = 0;
  let nb = 0;
...
export function buildImpression(
  ...
  const vector = Object.fromEntries(DIMENSIONS.map((d) => [d, 0])) as Record<Dimension, number>;
  ...
  const peak = Math.max(...DIMENSIONS.map((d) => vector[d]), 1e-9);
  for (const dim of DIMENSIONS) vector[dim] = vector[dim] / peak;
