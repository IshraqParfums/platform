...
import type { MaterialComposition } from "./affinity.js";
...
/** The client-safe slice of a material this tool needs. */
export interface AtelierMaterial {
  id: string;
  name: string;
  notePosition: NotePosition;
  /** 1–10; scales how loud a given dose reads. */
  strength: number;
  /** Hours until effectively gone. */
  tenacityHours: number;
  evapCurve: string;
  /** 6 (heaviest base) to 95 (flashiest top) — drives the onset term. */
  evapIndex: number;
  odour: string;
  /** Primary and secondary together — for search. */
  facets: string[];
  /** What the material *is*. Two materials sharing these are substitutes. */
  facetsPrimary: string[];
  /** What it also carries. Two materials sharing these are what melds. */
  facetsSecondary: string[];
  perceptualCluster: string[];
  /** −4 (cold) to +4 (hot). The palette's own reading. */
  temperature: number;
  /** −3 (transparent) to +4 (resinous) — how heavy the air feels. */
  polarity: number;
  /** Authored feeling words: "comfort", "distance", "ceremony". */
  emotion: string[];
  colourPercept: string;
  families: Partial<Record<Dimension, number>>;
  /** Dominant family, for the line colour. */
...
/** One line of the formula being built. */
export interface FormulaRow {
  materialId: string;
  /** Percent of the finished compound, neat basis. */
  neatPct: number;
}
