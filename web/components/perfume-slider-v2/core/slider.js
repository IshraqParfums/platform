...
export function createPerfumeSlider(root, options = {}) {
  ...
  /**
   * Hysteresis margins for the two state toggles below (visibility,
   ...
   * flipping every frame. A flip means a display change for visibility — a
   * repaint — which is what read as blinking while swiping.
   */
  ...
  function layout() {
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      const el = slides[i];
      const state = el._ipxState || (el._ipxState = { hidden: t < HIDE_BELOW, refl: t > 0.55 });
      ...
      // v1 blurred the background slides for depth of field, scaled by how
      // far round the table they sat. It was already the single most
      // expensive thing this component did per frame (blur cost scales
      // with the area blurred, and every drag frame touches several
      // slides' worth), tuned and re-tuned across rounds, then dropped on
      // touch only. This rebuild drops it everywhere: scale and opacity
      // alone already carry the depth read, and a slide's own filter
      // promotion (and the SVG-label repaint bug that came with it, see
      // z-index below) is gone with it.
